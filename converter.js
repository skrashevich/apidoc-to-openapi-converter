// apidoc-to-openapi.js
const fs = require('fs');
const path = require('path');
const vm = require('vm');

if (process.argv.length < 3) {
  console.error('Usage: node convrter.js <api_data.js> [output.json]');
  process.exit(1);
}

const apiDataPath = path.resolve(process.argv[2]);
const outputPath = process.argv[3] ? path.resolve(process.argv[3]) : null;
const raw = fs.readFileSync(apiDataPath, 'utf8');

let apiObject = null;
const sandbox = {
  define: (obj) => { apiObject = obj; },
  window: {}
};

vm.createContext(sandbox);
vm.runInContext(raw, sandbox, { filename: apiDataPath });

if (!apiObject || !Array.isArray(apiObject.api)) {
  console.error('Cannot find api array in api_data.js');
  process.exit(1);
}
...
for (const entry of apiEntries) {
  ...
  if (!openapi.paths[normalizedPath]) {
    openapi.paths[normalizedPath] = {};
  }

  if (openapi.paths[normalizedPath][method]) {
    console.warn(`Duplicate operation for ${method.toUpperCase()} ${normalizedPath}, skipping ${entry.name || 'unnamed'}`);
    continue;
  }
